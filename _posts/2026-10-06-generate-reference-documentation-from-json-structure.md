...
image: /social-cards/generate-reference-documentation-from-json-structure.png
description: >-
...
A hand-maintained page can omit declarations that remain present in the schema.
...
The [`s2md` command registration](https://github.com/clemensv/avrotize/blob/main/avrotize/commands.json)
connects that command to the Markdown converter. The
[`structuretomd.py` implementation](https://github.com/clemensv/avrotize/blob/main/avrotize/structuretomd.py)
...
build can reproduce the page and detect stale output. For a repository that
...
workflow explanation in an authored article. The next build will overwrite a
local patch to generated output, as it should.
