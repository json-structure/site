...
base-10 value domain. Calling it a `double` and printing two fractional digits
does not create that domain; it only hides the approximation on display. JSON
Structure therefore declares `decimal` separately from `float` and `double` and
...
`float` and `double` model IEEE 754 binary32 and binary64 values. They are useful
...
`decimal` makes the base-10 intent explicit before any arithmetic begins. A
...
arbitrary text: the `decimal` type requires the specified numeric grammar and
...
value. A schema with `scale: 2` makes the expected fractional capacity clear.
