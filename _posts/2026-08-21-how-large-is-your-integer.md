...
Call a field `integer` and the awkward questions arrive later. How wide is it?
...
The familiar `integer` type is an alias for `int32`. It exists for JSON Schema
...
That is why `int8` through `uint32` use unquoted JSON numbers, while `int64`,
`uint64`, `int128`, and `uint128` use strings. The string syntax follows the
...
JSON number can change it. Parsing the quoted representation as `uint64` cannot.
...
JSON Structure's quoted `int64` and `uint64` representation addresses that path
explicitly.
...
`integer`. The practical difference is the carrier: XML text naturally holds
...
For the example above, `nextSequence` is a `uint64` all the way from JSON text to
